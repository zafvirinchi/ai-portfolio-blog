...
// route made a real, uncapped OpenAI call (generateFollowUpEmail()) with
// NO session or entitlement check at all. Fixed to require a real
// session + recruiter.candidates entitlement before any lookup or LLM call.
...
const generateFollowUpEmailMock = vi.fn();
vi.mock("@/lib/ai/recruitment/notification-service", () => ({
  generateFollowUpEmail: (...args: unknown[]) => generateFollowUpEmailMock(...args),
}));
...
beforeEach(() => {
  ...
  generateFollowUpEmailMock.mockReset();
});

describe("POST /api/ai/recruitment/emails/follow-up — Phase 23 M5 cost defect fix", () => {
  it("PROVES zero LLM calls for an unauthenticated caller", async () => {
    ...
    const response = await POST(req({ pipelineCandidateId: "pc1" }));

    expect(response.status).toBe(401);
    expect(generateFollowUpEmailMock).not.toHaveBeenCalled();
  });

  it("PROVES zero LLM calls when the authenticated recruiter lacks recruiter.candidates", async () => {
    requireRecruiterIdMock.mockResolvedValue("real-recruiter");
    requireFeatureMock.mockRejectedValue(new FakeFeatureNotEntitledError("recruiter.candidates"));
  ...
  it("succeeds for a real, entitled recruiter session", async () => {
    ...
    generateFollowUpEmailMock.mockResolvedValue({ subject: "Checking in", body: "..." });

    const response = await POST(req({ pipelineCandidateId: "pc1" }));

    expect(response.status).toBe(200);
    expect(generateFollowUpEmailMock).toHaveBeenCalled();
  });
});
