...
// route made a real, uncapped OpenAI call (generateRejectionEmail())
...
const generateRejectionEmailMock = vi.fn();
vi.mock("@/lib/ai/recruitment/notification-service", () => ({
  generateRejectionEmail: (...args: unknown[]) => generateRejectionEmailMock(...args),
}));
...
beforeEach(() => {
  ...
  generateRejectionEmailMock.mockReset();
});

describe("POST /api/ai/recruitment/emails/rejection — Phase 23 M5 cost defect fix", () => {
  it("PROVES zero LLM calls for an unauthenticated caller", async () => {
    ...
    expect(generateRejectionEmailMock).not.toHaveBeenCalled();
  });
  ...
  it("succeeds for a real, entitled recruiter session", async () => {
    ...
    generateRejectionEmailMock.mockResolvedValue({ subject: "Update", body: "..." });

    const response = await POST(req({ pipelineCandidateId: "pc1" }));

    expect(response.status).toBe(200);
    expect(generateRejectionEmailMock).toHaveBeenCalled();
  });
});
