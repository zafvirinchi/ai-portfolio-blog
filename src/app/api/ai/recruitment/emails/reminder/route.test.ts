...
// route made a real, uncapped OpenAI call (generateInterviewReminderEmail())
...
const generateInterviewReminderEmailMock = vi.fn();
vi.mock("@/lib/ai/recruitment/notification-service", () => ({
  generateInterviewReminderEmail: (...args: unknown[]) => generateInterviewReminderEmailMock(...args),
}));
...
beforeEach(() => {
  ...
  generateInterviewReminderEmailMock.mockReset();
});

describe("POST /api/ai/recruitment/emails/reminder — Phase 23 M5 cost defect fix", () => {
  it("PROVES zero LLM calls for an unauthenticated caller", async () => {
    ...
    expect(generateInterviewReminderEmailMock).not.toHaveBeenCalled();
  });
  ...
  it("succeeds for a real, entitled recruiter session", async () => {
    ...
    generateInterviewReminderEmailMock.mockResolvedValue({ subject: "Reminder", body: "..." });

    const response = await POST(req({ pipelineCandidateId: "pc1", interviewId: "i1" }));

    expect(response.status).toBe(200);
    expect(generateInterviewReminderEmailMock).toHaveBeenCalled();
  });
});
