...
import { LinkedinSkillCategoryGroup, SKILLS_JSON_SCHEMA, SKILL_CATEGORIES, skillsLlmOutputSchema } from "./linkedin-schema";
 ...
 * followed. Ported from resume-rewriter/skills-rewriter.ts's
 * filterToActuallyPossessedSkills.
 */
function filterToActuallyPossessedSkills(categories: LinkedinSkillCategoryGroup[], resume: Resume): LinkedinSkillCategoryGroup[] {
...
export async function generateSkills(resume: Resume, correction?: string): Promise<LinkedinSkillCategoryGroup[]> {
  const completion = await openai.chat.completions.create({
    ...
    response_format: {
      type: "json_schema",
      json_schema: SKILLS_JSON_SCHEMA,
    },
  });
  ...
  if (!raw) {
    throw new Error("LinkedIn skills generation LLM returned no content");
  }

  const parsed = skillsLlmOutputSchema.safeParse(JSON.parse(raw));

  if (!parsed.success) {
    throw new Error(`LinkedIn skills generation failed schema validation: ${parsed.error.message}`);
  }
